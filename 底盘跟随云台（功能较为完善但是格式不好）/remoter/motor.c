...
{ 
	rc_dead_0 = - rc.ch0;
	rc_dead(&rc_dead_0);
	tar_tar += rc_dead_0 * 0.2;
  M6020_i_follow.realSpeed = motor_chassis[4].speed_rpm;
	change_imu_to_angele((int32_t) mpu6500_Exportdata.angle_degree[0]);
	M6020_t_follow.targetAngle = (start_imu + tar_tar);
	Handle_Angle8191_PID_Over_Zero(&M6020_t_follow.targetAngle,&M6020_t_follow.yaw_imu_angle);
...
void follow_motor_under(void) //底盘跟随(轮子)
{
	...
	rc_dead(&rc_dead_3);
		for(int i = 0; i<4; i++)
	{
		M3508_I_follow[0].targetSpeed = ( rc_dead_3 * 10 + rc_dead_2 * 10 + 12*M3508_P_follow.outCurrent); //1号轮子
		M3508_I_follow[1].targetSpeed = (-rc_dead_3 * 10 + rc_dead_2 * 10 + 12*M3508_P_follow.outCurrent); //2号轮子
		M3508_I_follow[2].targetSpeed = (-rc_dead_3 * 10 - rc_dead_2 * 10 + 12*M3508_P_follow.outCurrent); //3号轮子
		M3508_I_follow[3].targetSpeed = ( rc_dead_3 * 10 - rc_dead_2 * 10 + 12*M3508_P_follow.outCurrent); //4号轮子

		M3508_I_follow[i].realSpeed = motor_chassis[i].speed_rpm;
		M3508_I_follow[i].outCurrent = Incremental_PID(&M3508_I_follow[i].pid, M3508_I_follow[i].targetSpeed, M3508_I_follow[i].realSpeed); 
	}
...
{
	if(*rc_dead >= 0 && *rc_dead <= 40)
	{
		*rc_dead = 0;
	}
	else if(*rc_dead <= 0 && *rc_dead >= -40)
	{
		*rc_dead = 0;
	}
}
