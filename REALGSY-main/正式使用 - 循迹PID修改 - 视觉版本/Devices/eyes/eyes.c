...
void USART1_IRQHandler(void)
{
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) //接受数据
	{
		...
		if(runrun == 3)
		{
			...
			{
			  Target_velocity1 = -200;
			  Target_velocity2 = -500;
				Target_velocity3 =  200;
			...
			else if(res == 'R')
			{
			  ...
			  Target_velocity2 = -200;
				Target_velocity3 =  500;
				Target_velocity4 = -200;
				delay_ms(800);
				eread_sensor_values();
			}
	  }
...
void eread_sensor_values(void)
{
	 	...
	 	else	 if ((esensor[0] == 0) && (esensor[1] == 0) && (esensor[2] == 1) && (esensor[3] == 1) && (esensor[4] == 0)&& (esensor[5] == 0)&& (esensor[6] == 0))
   {
	 ...
	 else if ((esensor[0] == 1) && (esensor[1] == 1)&&(esensor[3] == 1) && (esensor[4] == 1)&& (esensor[5] == 1)&&(esensor[7] == 1)&& (esensor[8] == 1))
	 {
			stop();
	 }
